...
/* WILLIAMGAMES_NEW_GAMES_FINAL */
document.addEventListener("DOMContentLoaded",function(){
 const A=document.getElementById("aimGame"),F=document.getElementById("flappyGame"),H=document.getElementById("homeScreen");
 if(!A||!F)return;
 const show=s=>{document.querySelectorAll(".screen").forEach(x=>x.classList.remove("active"));s.classList.add("active")};
 document.querySelectorAll('[data-game="aim"]').forEach(b=>b.addEventListener("click",()=>show(A)));
 document.querySelectorAll('[data-game="flappy"]').forEach(b=>b.addEventListener("click",()=>show(F)));
 document.querySelectorAll("[data-wg-back]").forEach(b=>b.addEventListener("click",()=>show(H)));

 const arena=document.getElementById("aimArena"),t=document.getElementById("aimTarget"),st=document.getElementById("aimStart"),ov=document.getElementById("aimStartOverlay");
 const score=document.getElementById("aimScore"),hitsEl=document.getElementById("aimTargets"),acc=document.getElementById("aimAccuracy"),as=document.getElementById("aimStatus");
 let running=false,hits=0,shots=0,total=0,started=0;
 function move(){let x=12+Math.random()*(arena.clientWidth-66),y=12+Math.random()*(arena.clientHeight-66);t.style.left=x+"px";t.style.top=y+"px";t.classList.remove("hidden");started=performance.now()}
 st.addEventListener("click",()=>{running=true;hits=shots=total=0;score.textContent=0;hitsEl.textContent=0;acc.textContent="100%";as.textContent="HIT THE TARGET!";ov.classList.add("hidden");move()});
 arena.addEventListener("click",e=>{if(running&&e.target!==t){shots++;acc.textContent=Math.round(hits/Math.max(1,shots)*100)+"%"}});
 t.addEventListener("click",e=>{e.stopPropagation();if(!running)return;hits++;shots++;let bonus=Math.max(0,Math.round(5000/Math.max(1,performance.now()-started)));total+=100+bonus;score.textContent=total;hitsEl.textContent=hits;acc.textContent=Math.round(hits/shots*100)+"%";if(hits===10){running=false;t.classList.add("hidden");as.textContent="10 TARGETS! BASE + SPEED = "+total;ov.classList.remove("hidden")}else move()});

 const c=document.getElementById("flappyCanvas"),ctx=c.getContext("2d"),fs=document.getElementById("flappyStart"),fo=document.getElementById("flappyOverlay"),fscore=document.getElementById("flappyScore"),fst=document.getElementById("flappyStatus");
 let col="blue",fr=0,fy=240,fv=0,ps=[],playing=false,sc=0,raf=null; const cols={blue:"#2868dd",red:"#e33",purple:"#8b46cc",green:"#2bb34f"};
 document.querySelectorAll(".ninja-choice").forEach(b=>b.addEventListener("click",()=>{document.querySelectorAll(".ninja-choice").forEach(x=>x.classList.remove("selected"));b.classList.add("selected");col=b.dataset.ninja;fst.textContent="NINJA: "+col.toUpperCase()+" — PRESS START"}));
 document.querySelector(".ninja-choice.blue")?.classList.add("selected");
 function draw(){ctx.fillStyle="#050508";ctx.fillRect(0,0,400,500);ctx.fillStyle="#11111b";for(let x=0;x<400;x+=20)ctx.fillRect(x,0,1,500);for(let y=0;y<500;y+=20)ctx.fillRect(0,y,400,1);ctx.fillStyle="#35bd52";ps.forEach(p=>{ctx.fillRect(p.x,0,48,p.top);ctx.fillRect(p.x,p.bottom,48,500-p.bottom)});ctx.fillStyle=cols[col];ctx.fillRect(70,fy,24,22);ctx.fillStyle="#050508";ctx.fillRect(66,fy+4,8,5);ctx.fillStyle="#fff";ctx.fillRect(87,fy+5,4,4);ctx.fillStyle=cols[col];ctx.fillRect(74,fy+20,6,6);ctx.fillRect(86,fy+20,6,6)}
 function stop(){playing=false;if(raf)cancelAnimationFrame(raf);raf=null}
 function flap(){if(playing)fv=-6.5}
 function start(){stop();playing=true;fr=0;fy=240;fv=0;ps=[];sc=0;fscore.textContent=0;fst.textContent="FLAP!";fo.classList.add("hidden");raf=requestAnimationFrame(loop)}
 function loop(){if(!playing)return;fr++;fv+=.34;fy+=fv;if(fr%95===0){let top=60+Math.random()*235;ps.push({x:410,top:top,bottom:top+135,counted:false})}ps.forEach(p=>p.x-=2.8);ps=ps.filter(p=>p.x>-60);ps.forEach(p=>{if(!p.counted&&p.x<70){p.counted=true;sc++;fscore.textContent=sc}});draw();let hit=fy<0||fy>478||ps.some(p=>94>p.x&&70<p.x+48&&(fy<p.top||fy+26>p.bottom));if(hit){stop();fst.textContent="GAME OVER — SCORE "+sc;fo.classList.remove("hidden");return}raf=requestAnimationFrame(loop)}
 fs.addEventListener("click",e=>{e.stopPropagation();start()});c.addEventListener("click",()=>playing?flap():start());document.addEventListener("keydown",e=>{if(e.code==="Space"&&F.classList.contains("active")){e.preventDefault();playing?flap():start()}});draw();
});
